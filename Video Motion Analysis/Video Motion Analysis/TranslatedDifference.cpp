...
delta DetermineBulkImageTranslation(unsigned char* frame1, unsigned char* frame2, int width, int height, int downsample, int downsampled_shift) {
...
unsigned char* TranslatedDifference(unsigned char* frame1, unsigned char* frame2, int width, int height, int shift) {
	delta d = DetermineBulkImageTranslation(frame1, frame2, width, height, 2, 8);
	
	std::cout << "delta x: " << d.x << " delta y: " << d.y << std::endl;
	...
	for (int i = 0; i < width * height * 4; i += 4) {
		...
		for (int x = -shift; x <= shift; x++) {
			for (int y = -shift; y <= shift; y++) {
				...
				int diff_x = i % (width * 4) + x * 4 + d.x;
				int diff_y = i / (width * 4) + y + d.y;
